...
class GraphOperationsAStar:
    ...
    def heauristicWithButter(self,state , whichButter,  whichPerson ):
        buttersLoc=state.getButters()[whichButter].getLocation()
        personsLoc =self.__persons[whichPerson].getLocation()
        return  abs(buttersLoc[0] - personsLoc[0]) + abs(buttersLoc[1] - personsLoc[1])

    def heauristicWithoutButter(self,state , whichButter ):
        buttersLoc=state.getButters()[whichButter].getLocation()
        robotsLoc=state.getRobot().getLocation()
        return  abs(buttersLoc[0] - robotsLoc[0]) + abs(buttersLoc[1] - robotsLoc[1])

...
public class Controller implements Initializable
{
    ...
    String pathReal = " LRRDUDD";

    @FXML
    void startDrawing(ActionEvent event)
    {
        int row = 5;
        int column = 5;
        String path = Character.toString(pathReal.charAt(0));
        pathReal = pathReal.substring(1);

        for (int p = 0; p < path.length(); p++)
        {
            temp.setVisible(false);
            temp.setVisible(true);
            char m = path.charAt(p);
